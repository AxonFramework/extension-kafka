...
public class KafkaAdminUtils {
    public static void createTopics(KafkaContainer kafkaContainer, KafkaTopicConfiguration... topics) {
    ...
    public static List<NewTopic> topics(KafkaTopicConfiguration... topics) {
        return Arrays.stream(topics)
                .map(topic -> new NewTopic(topic.getName(), topic.getNumPartitions(), topic.getReplicationFactor()))
                .collect(Collectors.toList());
    }

    public static Map<String, Object> minimalAdminConfig(KafkaContainer kafkaContainer) {
        Map<String, Object> configs = new HashMap<>();
        configs.put(CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG, kafkaContainer.getBootstrapServers()); // kafka instances
        return configs;
    }
